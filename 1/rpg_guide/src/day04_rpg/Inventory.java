	...
	public void equipMenu() {
		if(itemList.size()!=0) {
			...
			while (true) {
				...
				if (selEquip == 0)
					break;
				if (itemList.get(selEquip-1).kind == Item.WEAPON) {
					if (Player.getGuildUnit(selUnit - 1).weapon != null) {
						itemList.add(Player.getGuildUnit(selUnit - 1).weapon);
					}
					Player.getGuildUnit(selUnit - 1).weapon = itemList.get(selEquip-1);
					Player.getGuildUnit(selUnit - 1 ).att +=Player.inven.itemList.get(selEquip-1).power;
				} else if (itemList.get(selEquip-1).kind == Item.ARMOR) {
					if (Player.getGuildUnit(selUnit - 1).armor != null) {
						itemList.add(Player.getGuildUnit(selUnit - 1).armor);
					}
					Player.getGuildUnit(selUnit - 1).armor = itemList.get(selEquip-1);
					Player.getGuildUnit(selUnit-1).def += Player.inven.itemList.get(selEquip-1).power;
				} else if (itemList.get(selEquip-1).kind == Item.RING) {
					if (Player.getGuildUnit(selUnit - 1).ring != null) {
						itemList.add(Player.getGuildUnit(selUnit - 1).ring);
					}
					Player.getGuildUnit(selUnit - 1).ring = itemList.get(selEquip-1);
					Player.getGuildUnit(selUnit-1).maxHp += Player.inven.itemList.get(selEquip-1).power;
					Player.getGuildUnit(selUnit-1).hp += Player.inven.itemList.get(selEquip-1).power;
				}
